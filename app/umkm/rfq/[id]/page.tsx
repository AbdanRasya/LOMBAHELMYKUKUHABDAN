  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/umkm/rfq">
          <Button variant="ghost" size="sm" className="gap-1 pl-0 text-slate-600 hover:text-slate-900"><ArrowLeft className="h-4 w-4" />Pasar RFQ</Button>
        </Link>
          ...
          <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            {(rfq.budgetMin || rfq.budgetMax) && (
              <div className="flex items-start gap-2">
                ...
                <div><p className="text-xs text-slate-400">Estimasi Budget</p><p className="text-sm font-medium">{rfq.budgetMin ? formatRp(rfq.budgetMin) : ""}{rfq.budgetMax ? ` – ${formatRp(rfq.budgetMax)}` : ""}</p></div>
              </div>
            )}
            {rfq.quantity && (
              <div className="flex items-start gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          {existingQuotation ? (
            <Card className="border-emerald-200 bg-emerald-50/50 dark:bg-emerald-950/20 border shadow-sm">
              ...
              <CardContent className="space-y-3 text-sm text-emerald-900 dark:text-emerald-200">
                <div>
                  <p className="text-xs text-emerald-700/60 dark:text-emerald-400/60">Harga Penawaran</p>
                  <p className="text-lg font-bold">{formatRp(existingQuotation.price)}</p>
                </div>
                {existingQuotation.leadTimeDays && (
                  <div>
                    <p className="text-xs text-emerald-700/60 dark:text-emerald-400/60">Lead Time</p>
                    <p className="font-semibold">{existingQuotation.leadTimeDays} Hari</p>
                  </div>
                )}
                <div>
                  <p className="text-xs text-emerald-700/60 dark:text-emerald-400/60">Status</p>
                  <Badge className="bg-emerald-100 text-emerald-800 border-0 mt-1 uppercase text-[10px]">{existingQuotation.status}</Badge>
                </div>
              </CardContent>
            </Card>
          ) : (
            <SubmitQuotationForm rfqId={rfq.id} />
          )}
        </div>
      </div>
    </div>
