...
export async function GET() {
  ...
  try {
    ...
    if (role === 'COMPANY') {
      ...
      // Attach UMKM profile info
      const enriched = await Promise.all(conversations.map(async (c) => {
        const umkm = await db.umkmProfile.findUnique({ where: { userId: c.umkmUserId }, select: { businessName: true, logo: true, id: true } });
      ...
      const enriched = await Promise.all(conversations.map(async (c) => {
        const company = await db.companyProfile.findUnique({ where: { userId: c.companyUserId }, select: { companyName: true, logo: true, id: true } });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { otherUserId, rfqId } = body; // otherUserId is the other party's userId
    const role = (session.user as any).role;
